...
public class EnhancedCat extends EntityAnimal {
    ...
    public EnhancedCat(World worldIn) {
        super(worldIn);
    ...
    //TODO put new cat behaviour here 
...
//    }
        ...
        mixMateMitosisGenes();
        mixMitosisGenes();
        EnhancedCat enhancedcat = new EnhancedCat(this.world);
        enhancedcat.setGrowingAge(0);
        int[] babyGenes = getKittenGenes();
    ...
    public int[] getKittenGenes() {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (thisOrMate) {
                kittenGenes[i] = genes[i];
            } else {
                kittenGenes[i] = mateGenes[i];
            }
        }
        ...
        for (int i = 20; i < genes.length; i++) {
            boolean thisOrMate = rand.nextBoolean();
            if (thisOrMate) {
                kittenGenes[i] = mitosisGenes[i];
            } else {
                kittenGenes[i] = mateMitosisGenes[i];
            }
        }
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
        ...
        if (livingdata instanceof EnhancedCat.GroupData) {
            spawnGenes = ((EnhancedCat.GroupData) livingdata).groupGenes;
        } else {
            spawnGenes = createInitialGenes();
            livingdata = new EnhancedCat.GroupData(spawnGenes);
        }
