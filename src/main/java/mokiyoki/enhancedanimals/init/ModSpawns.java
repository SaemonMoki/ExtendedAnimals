    ...
    public record BiomeSpawnModifier() implements BiomeModifier {
        ...
        private void addGeneticAnimals(Holder<Biome> biomeHolder, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
            ...
            while (currentSpawns.hasNext()) {
                SpawnerData entry = currentSpawns.next();
                ...
                if (entry.type == EntityType.PIG) {
                    if(config.spawnGeneticPigs.get()) {
                        addSpawns.add(new SpawnerData(ENHANCED_PIG.get(), config.spawnWeightPigs.get(), config.minimumPigGroup.get(), config.maximumPigGroup.get()));
                    }
                }
                //add sheep
                if (entry.type == EntityType.SHEEP) {
                    if (config.spawnGeneticSheep.get()) {
                        addSpawns.add(new SpawnerData(ENHANCED_SHEEP.get(), config.spawnWeightSheep.get(), config.minimumSheepGroup.get(), config.maximumSheepGroup.get()));
                    }
                }
                //add cow
                if (entry.type == EntityType.COW) {
                    if(config.spawnGeneticCows.get()) {
                        addSpawns.add(new SpawnerData(ENHANCED_COW.get(),config.spawnWeightCows.get(), config.minimumCowGroup.get(), config.maximumCowGroup.get()));
                    }
                }
                //add llama
                if (entry.type == EntityType.LLAMA) {
                    if(config.spawnGeneticLlamas.get()) {
                        addSpawns.add(new SpawnerData(ENHANCED_LLAMA.get(), config.spawnWeightLlamas.get(), config.minimumLlamaGroup.get(), config.maximumLlamaGroup.get()));
                    }
                }
                //add chicken
                if (entry.type == EntityType.CHICKEN) {
                    if (config.spawnGeneticChickens.get()) {
                        addSpawns.add(new SpawnerData(ENHANCED_CHICKEN.get(), config.spawnWeightChickens.get(), config.minimumChickenGroup.get(), config.maximumChickenGroup.get()));
                    }
                }
                //add rabbit
                if (entry.type == EntityType.RABBIT) {
                    if (config.spawnGeneticRabbits.get()) {
                        addSpawns.add(new SpawnerData(ENHANCED_RABBIT.get(), config.spawnWeightRabbits.get(), config.minimumRabbitGroup.get(), config.maximumRabbitGroup.get()));
                    }
                }
                //add mooshroom
                if (entry.type == EntityType.MOOSHROOM) {
                    if (config.spawnGeneticMooshroom.get()) {
                        addSpawns.add(new SpawnerData(ENHANCED_MOOSHROOM.get(), config.spawnWeightMooshrooms.get(), config.minimumMooshroomGroup.get(), config.maximumMooshroomGroup.get()));
                    }
                }
                //add moobloom
                if (entry.type.toString().contains("moobloom")) {
                    if (config.spawnGeneticMoobloom.get()) {
                        addSpawns.add(new SpawnerData(ENHANCED_MOOBLOOM.get(), entry.getWeight(), entry.minCount, entry.maxCount));
                    }
                }
                if (entry.type == EntityType.TURTLE) {
                    if(config.spawnGeneticTurtles.get()) {
                        addSpawns.add(new SpawnerData(ENHANCED_TURTLE.get(), config.spawnWeightTurtles.get(), config.minimumTurtleGroup.get(), config.maximumTurtleGroup.get()));
                    }
                }
                if (entry.type == EntityType.AXOLOTL) {
                    if(config.spawnGeneticAxolotls.get()) {
                        addSpawns.add(new SpawnerData(ENHANCED_AXOLOTL.get(), config.spawnWeightAxolotls.get(), config.minimumAxolotlGroup.get(), config.maximumAxolotlGroup.get()));
                    }
                }
            }
            ...
            ArrayList<SpawnerData> addAxolotlSpawns = new ArrayList<>();

            Iterator<SpawnerData> currentAxolotlSpawns = builder.getMobSpawnSettings().getSpawner(MobCategory.AXOLOTLS).iterator();
            while (currentAxolotlSpawns.hasNext()) {
                SpawnerData entry = currentAxolotlSpawns.next();

                GeneticAnimalsConfig.CommonConfig config = GeneticAnimalsConfig.COMMON;

                if (entry.type == EntityType.AXOLOTL) {
                    if(config.spawnGeneticAxolotls.get()) {
                        addAxolotlSpawns.add(new SpawnerData(ENHANCED_AXOLOTL.get(), config.spawnWeightAxolotls.get(), config.minimumAxolotlGroup.get(), config.maximumAxolotlGroup.get()));
                    }
                }
            }

            List<SpawnerData> axolotlSpawns = builder.getMobSpawnSettings().getSpawner(MobCategory.AXOLOTLS);
            ...
            if (biomeHolder.containsTag(Tags.Biomes.IS_SNOWY)) {
                spawns.add(new SpawnerData(ENHANCED_RABBIT.get(), GeneticAnimalsConfig.COMMON.spawnWeightRabbits.get(), GeneticAnimalsConfig.COMMON.minimumRabbitGroup.get(), GeneticAnimalsConfig.COMMON.maximumRabbitGroup.get()));
            }
        }

        private void removeVanillaAnimals(ModifiableBiomeInfo.BiomeInfo.Builder builder) {
            ArrayList<SpawnerData> removeSpawns = new ArrayList<>();
            ...
            ArrayList<SpawnerData> removeAxolotlSpawns = new ArrayList<>();
