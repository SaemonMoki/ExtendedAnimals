...
public class ChickenGeneticsInitialiser extends AbstractGeneticsInitialiser {
    ...
    public ChickenGeneticsInitialiser() {
        ...
        this.breeds.add(ChickenBreeds.SILVER_LIGHT_BRAHMA);
        this.breeds.add(ChickenBreeds.GOLD_LIGHT_BRAHMA);
        ...
        this.breeds.add(ChickenBreeds.SERAMA);
        this.breeds.add(ChickenBreeds.CHABO);
...
//             TODO here: genes for egg hatch chance when thrown, egg laying rate, and chicken ai modifiers
        ...
        //"tailless gene"
        sexlinkedGenes[20] = ThreadLocalRandom.current().nextInt(100) > WTC ? (ThreadLocalRandom.current().nextInt(2) + 1) : 1;
        sexlinkedGenes[21] = ThreadLocalRandom.current().nextInt(100) > WTC ? (ThreadLocalRandom.current().nextInt(2) + 1) : 1;
        ...
        // 10-19 neck angle
        for (int i = 0; i < 20; i++) {
            autosomalGenes[i] = ThreadLocalRandom.current().nextInt(100) > WTC ? (ThreadLocalRandom.current().nextInt(2) + 1) : 1;;
        }
        ...
        if (ThreadLocalRandom.current().nextInt(100) > WTC || wildType == 4) {
            autosomalGenes[33] = ThreadLocalRandom.current().nextBoolean() ? 1 : 3;
        ...
        //Extra tail feathers
        autosomalGenes[278] = ThreadLocalRandom.current().nextInt(100) > WTC ? (ThreadLocalRandom.current().nextInt(3) + 1) : 1;
        autosomalGenes[279] = ThreadLocalRandom.current().nextInt(100) > WTC ? (ThreadLocalRandom.current().nextInt(3) + 1) : 1;

        //"non molting gene"
        autosomalGenes[280] = ThreadLocalRandom.current().nextInt(100) > WTC ? (ThreadLocalRandom.current().nextInt(2) + 1) : 1;
        autosomalGenes[281] = ThreadLocalRandom.current().nextInt(100) > WTC ? (ThreadLocalRandom.current().nextInt(2) + 1) : 1;

        //short tailfeather gene
