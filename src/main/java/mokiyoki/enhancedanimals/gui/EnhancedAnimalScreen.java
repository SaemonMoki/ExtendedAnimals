...
public class EnhancedAnimalScreen extends AbstractContainerScreen<EnhancedAnimalContainer> {
    ...
    protected void init() {
        ...
        this.cBox = new Checkbox(photoI-44, photoJ+193, 20, 20, Component.translatable("photomode.stickerstyle"), false, false);
    ...
    private void renderInfoToolTip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        if (!this.photoModeEnabled) {
            ...
            if (this.isHovering(136, 5, 8, 9, (double)mouseX, (double)mouseY)) {
                guiGraphics.renderTooltip(this.font, Component.translatable("eanimod.animalinfocontainer.health").append(" " + (enhancedAnimalInfo.health*10)+"%"), mouseX, mouseY);
            }
            if (this.isHovering(147, 5, 8, 9, (double)mouseX, (double)mouseY)) {
                guiGraphics.renderTooltip(this.font, Component.translatable("eanimod.animalinfocontainer.hunger").append(" " + (enhancedAnimalInfo.hunger*10)+"%"), mouseX, mouseY);
            }
            ...
            if (!this.chestTabEnabled) {
                ...
                if (this.isHovering(100, 40, 50, 9, (double)mouseX, (double)mouseY)) {
                    guiGraphics.renderTooltip(this.font, Component.translatable("eanimod.animalinfocontainer.dam").append(": "+enhancedAnimalInfo.dam), mouseX, mouseY);
                }
            }
        } else {
            ...
            if (this.isHoveringPhotoMode(-106, 125, 27, 23, (double)mouseX, (double)mouseY)) {
                guiGraphics.renderTooltip(this.font, Component.translatable("eanimod.animalinfocontainer.photomode.stickerstyle"), mouseX, mouseY);
            }
            ...
            if (this.isHoveringPhotoMode(-36, -25, 28, 31, (double)mouseX, (double)mouseY)) {
                guiGraphics.renderTooltip(this.font, Component.translatable("eanimod.animalinfocontainer.photomode.takephoto"), mouseX, mouseY);
            }
        }
    }
...
//                fileList.sort(Comparator.comparing(File::getName));
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
        ...
        if (!photoModeEnabled) {
            ...
            guiGraphics.blit(GUI_TEXTURE, i + 136, j + 5, emptyIconPos[0], this.imageHeight + emptyIconPos[1], 9, 10); // health icon
            guiGraphics.blit(GUI_TEXTURE, i + 147, j + 5, emptyIconPos[2], this.imageHeight + emptyIconPos[3], 9, 10); // hunger icon
            guiGraphics.blit(GUI_TEXTURE, i + 158, j + 5, emptyIconPos[4], this.imageHeight + emptyIconPos[5], 10, 10); // tameness icon
...
//            int tameness = enhancedAnimalInfo.tameness;
            ...
            guiGraphics.blit(GUI_TEXTURE, i + 136, j + 5 + (10-health), fullIconPos[0], this.imageHeight + fullIconPos[1] + (10-health), 9, health); // health icon
            guiGraphics.blit(GUI_TEXTURE, i + 147, j + 5 + (10-hunger), fullIconPos[2], this.imageHeight + fullIconPos[3] + (10-hunger), 9, hunger); // hunger icon
